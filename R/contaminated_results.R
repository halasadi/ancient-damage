...
#fu <- get(load("../data/Fu_2016/Fu_2016.rda"))
sherpa <- get(load("../data/Sherpa_data/Sherpa_data.rda"))
...
pooled_data <- rbind(contam_samp, ancient, moderns, lazaridis, sherpa)
labs <- c(rep("contaminated", 5), rep("ancient", dim(ancient)[1]),
          rep("moderns", 50),
          rep("lazaridis", dim(lazaridis)[1]),
        #  rep("fu", dim(fu)[1]),
          rep("sherpa", dim(sherpa)[1]))
...
topic_clus <- maptpx::topics(pooled_data, K=4, type="independent", tol=50,
...
output_dir <- "../utilities/contaminated_modern_Pinhasi_lazaridis_sherpa/"
