...
################### contaminated results 2   ###############################


ancient <- get(load("../data/Pinhasi/Pinhasi.rda"))
moderns <- get(load("../data/moderns_lite/moderns_lite.rda"))
lazaridis <- get(load("../data/Lazaridis/Lazaridis.rda"))

samp_1 <- rmultinom(1, 9000, ancient[1,]) + rmultinom(1, 1000, moderns[1,])
samp_2 <- rmultinom(1, 7000, ancient[2,]) + rmultinom(1, 3000, moderns[20,])
samp_3 <- rmultinom(1, 5000, ancient[3,]) + rmultinom(1, 5000, moderns[30,])
samp_4 <- rmultinom(1, 3000, ancient[4,]) + rmultinom(1, 7000, moderns[40,])
samp_5 <- rmultinom(1, 1000, ancient[5,]) + rmultinom(1, 9000, moderns[50,])

moderns_filtered <- moderns[-c(1, 20, 30, 40, 50),]
moderns_filtered <- moderns_filtered[1:10,]
ancient_filtered <- ancient[-(1:5),]
contam_samp <- t(cbind(samp_1, samp_2, samp_3, samp_4, samp_5))

pooled_data <- rbind(contam_samp, ancient_filtered, moderns_filtered)

labs <- c(rep("contaminated", 5), rep("ancient", dim(ancient_filtered)[1]),
          rep("moderns", dim(moderns_filtered)[1]))
       #   rep("lazaridis", dim(lazaridis)[1]))
...
output_dir <- "../utilities/contaminated_moderns_Pinhasi/clus_2/"
...
structure.control.default <- list(yaxis_label = "aRchaic pops",
                                  order_sample = FALSE,
                                  figure_title = paste0("StructurePlot:"),
