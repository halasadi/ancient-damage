---
title: "152 ancients + Lindo 2016 data analysis"
author: "Kushal K Dey"
date: "9/29/2016"
output: 
  html_document:
...
ll <- flashr::flashpool(voom_signature_counts, K=10, 
                        ...
                        nonnegative=FALSE)
save(ll, file="../rda/flash_152ancients_lindo2016_pooled_k_10.rda")
```

```{r echo=TRUE, eval=TRUE}
ll <- get(load("../rda/flash_152ancients_lindo2016_pooled_k_10.rda"))
...
annotation <- data.frame(
  ...
  label = factor(c(rep("Ancient_Lindo",25),
                                       rep("Modern_Lindo",25),
                                       rep("I",length(I_labels)), rep("RISE",length(RISE_labels))))
)
...
```{r echo=TRUE, eval=FALSE}
topics_clus <- CountClust::FitGoM(signature_counts, K=2:6, tol=0.1, control=list(tmax=10000));

save(topics_clus, file="../rda/topics_clus_153ancients_lindo2016_pooled_2_to_6.rda")
```

```{r echo=TRUE, eval=TRUE}
topics_clus <- get(load("../rda/topics_clus_153ancients_lindo2016_pooled_2_to_6.rda"))
```
